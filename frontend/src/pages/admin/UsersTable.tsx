  ...
  return (
    <div className="bg-white dark:bg-neutral-900 border-2 border-black dark:border-neutral-700 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] overflow-hidden">
      <div className="px-4 sm:px-6 py-4 border-b-2 border-slate-200 dark:border-neutral-700 flex items-center gap-3">
        ...
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('admin.users')}</h2>
        {loading && (
          <span className="text-sm text-slate-500 dark:text-neutral-500 font-medium">{t('common.loading')}</span>
        )}
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 dark:bg-neutral-800/70">
            <tr className="text-left">
              <th className="px-4 sm:px-6 py-3 font-bold text-slate-600 dark:text-neutral-300">{t('admin.user')}</th>
              <th className="px-4 sm:px-6 py-3 font-bold text-slate-600 dark:text-neutral-300">{t('admin.role')}</th>
              <th className="px-4 sm:px-6 py-3 font-bold text-slate-600 dark:text-neutral-300">{t('admin.accountStatus')}</th>
              <th className="px-4 sm:px-6 py-3 font-bold text-slate-600 dark:text-neutral-300">{t('admin.mustReset')}</th>
              <th className="px-4 sm:px-6 py-3 font-bold text-slate-600 dark:text-neutral-300">{t('admin.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user) => (
              <tr key={user.id} className="border-t border-slate-100 dark:border-neutral-800">
                ...
                <td className="px-4 sm:px-6 py-4">
                  ...
                  >
                    {user.mustResetPassword ? t('admin.yes') : t('admin.no')}
                  </button>
                </td>
                <td className="px-4 sm:px-6 py-4">
                    ...
                    <button
                      ...
                      title={
                        user.id === currentUserId
                          ? t('admin.useProfileForOwnPassword')
                          : t('admin.generateTempPassword')
                      }
                    >
                      <KeyRound size={16} />
                      {resetPasswordLoadingId === user.id ? t('admin.generating') : t('admin.resetPassword')}
                    </button>
                  </div>
                </td>
              </tr>
